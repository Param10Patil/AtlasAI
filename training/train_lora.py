...
def train(
    ...
    *,
    ...
    result: dict[str, Any] = {'mode': mode, 'metadata': metadata, 'metrics': metrics, 'artifact_dir': str(output_dir), 'mlflow': {'status': 'disabled'}}
    if tracking_uri:
        try:
            import mlflow
            mlflow.set_tracking_uri(tracking_uri)
            experiment_name = os.getenv('OPSPILOT_MLFLOW_EXPERIMENT', 'OpsPilot training')
            mlflow.set_experiment(experiment_name)
            with mlflow.start_run(run_name=f'{mode}-{metadata["dataset_version"]}') as active_run:
                ...
                mlflow.set_tags({'artifact_schema': ARTIFACT_SCHEMA_VERSION, 'dataset_version': metadata['dataset_version'], 'mode': mode})
                run_record = {
                ...
                (output_dir / 'mlflow_run.json').write_text(json.dumps(run_record, indent=2) + '\n', encoding='utf-8')
                mlflow.log_artifacts(str(output_dir), artifact_path='artifact')
            result['mlflow'] = run_record
        except Exception as exc:  # noqa: BLE001 - optional tracking must not discard local evidence
            result['mlflow'] = {'status': 'warning', 'message': f'unavailable ({type(exc).__name__})'}
    return result
