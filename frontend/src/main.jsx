...
function ActionPreview({ remediation }) {
  ...
  return <div className="operation-preview"><div className="operation-head"><div><p className="eyebrow">EXECUTION PREVIEW</p><strong>What AtlasAI will execute</strong></div><Badge tone="neutral">MCP -&gt; Kubernetes API</Badge></div><p className="operation-purpose">{textValue(preview.operation, 'A policy-approved operation for the observed workload.')}</p><dl className="operation-facts"><div><dt>Action</dt><dd>{humanize(preview.action)}</dd></div><div><dt>Category</dt><dd>{textValue(preview.category, 'Not reported')}</dd></div><div><dt>Target</dt><dd>{textValue(preview.target, 'Allowlisted workload')}</dd></div><div><dt>Namespace</dt><dd>{textValue(preview.namespace, 'Not observed')}</dd></div><div><dt>Resource</dt><dd>{textValue(preview.resource_type, 'Kubernetes resource')}</dd></div><div><dt>API method</dt><dd><code>{textValue(preview.api_method, 'Kubernetes client method not reported')}</code></dd></div><div><dt>Policy</dt><dd>{textValue(preview.policy_result, 'Policy decision not reported')}</dd></div></dl><div className="operation-executor"><span className="footer-key">EXECUTION PATH</span><span>{textValue(preview.execution_method, 'MCP safe action through the Kubernetes Python client')}</span></div>{evidence.length > 0 && <details className="preview-evidence"><summary>Retrieved RAG evidence ({evidence.length})</summary><p className="preview-source-note">Reference only · evidence that informed the recommendation, not an execution instruction.</p><ul>{evidence.map((item, index) => <li key={(item.id || item.title || 'evidence') + '-' + index}><div><strong>{textValue(item.title, 'Retrieved source')}</strong><span>{Math.round(clamp(item.score) * 100)}% match</span></div><p>{textValue(item.excerpt, 'No excerpt supplied.')}</p></li>)}</ul></details>}{commands.length > 0 && <details className="preview-commands"><summary>RAG suggested commands ({commands.length})</summary><p className="preview-source-note">Reference only · these commands were retrieved from runbooks and are not executed directly.</p>{commands.map((command, index) => <code key={command + '-' + index}>{command}</code>)}</details>}<p className="operation-note"><Dot state="good" /> The executor calls the allowlisted Kubernetes client operation through MCP. AtlasAI does not execute arbitrary shell or kubectl commands.</p></div>;
}
