...
function App() {
  ...
  return <div className="app-frame"><Sidebar page={page} setPage={setPage} runtime={runtime} open={sidebarOpen} onClose={() => setSidebarOpen(false)} /><div className="app-main"><Header runtime={{ ...runtime, onMenu: () => setSidebarOpen(true) }} /><main className="page-shell">{content}</main><footer className="app-footer"><span>OpsPilot</span><span>Policy-gated infrastructure assistance</span></footer></div>{confirmScenario && <ConfirmationModal scenario={scenario} onConfirm={inject} onCancel={() => setConfirmScenario(false)} />}</div>;
}
